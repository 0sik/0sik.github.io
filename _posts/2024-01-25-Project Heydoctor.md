...
한국공학대학교 Graduation Project이다. AI 챗봇을 이용한 건강 자가진단 및 주변 병원 온라인 예약 서비스인데, AI 챗봇 서버는 Docker을 활용하여 Flask 서버이고, 카카오톡 디벨로퍼를 사용하여 카카오톡에서 요청이 오면 챗봇 서버로 요청이 가게 설계했다.
그리고 자기 자신 주변 병원 위치나 운영시간 등등을 알 수 있는 웹사이트 하나 만들었다.

이 프로젝트에서는 인프라 쪽을 대부분을 맡아서 진행하였다.

CICD나 HTTPS같은 기본 개념적인거를 정리하기 보다 내가 인프라를 설계하면서 얻은 이점같은것들을 정리해보자

NGINX을 이용

사용자의 접속은 HTTPS로 NGINX에서 먼저 처리된다. NGINX는 정적 요청은 미리 빌드된 프론트엔드 파일로 바로 응답하고, 동적 요청은 백엔드 애플리케이션으로 리버스 프록시한다. (예: 웹 서비스 API는 Django, 카카오톡 챗봇 API는 Flask로 라우팅)
3명이서 하는 프로젝트다 보니 배포 역할을 한명이 담당해야 했는데 그러다보니 단일 서버만 이용할수밖에없었다. 그래서 nginx을 이용하여 단일서버에 프론트 백 데이터베이스까지 docker compose를 이용하여 비용을 감소하였다.
하지만 이러한 방법은 안좋을수 있다. 확장성이 제약될수있고, 프론트 백이 격리되지 않아 문제가 발생하면 다른 구성에 영향을 미칠 수 있다.
...
데이터베이스(MySQL)도 Docker Compose로 컨테이너로 띄워 RDS를 사용하지 않았다. 이 방식은 관리형 DB 비용을 줄일 수 있는 선택지이지만, 그만큼 백업/복구, 장애 대응, 고가용성, 보안 패치 같은 운영 책임이 직접 생긴다.
그래서 “비용 절감”은 가능하되, 실무에서는 비용 외에 운영 리스크/가용성 요구사항까지 같이 보고 결정하게 된다.
...
하지만 다른 관점에서 지켜볼 필요가 있다. 이러한 단점들을 커버할만한 이점이 있어서 쓰는것 아니겠나! 맨 처음 작성한 학교 졸업작품에서 적용해보자.

프론트 앤드에 nginx가 있고 요청이 들어오면 flask서버로 라우팅한다 flask서버가 호출되면 데이터베이스를 호출한다.
dockercompose를 사용한다면 단일 서비스가 아닌 이 모든 서비스를 관리 할 수 있게 된다. 각 서비스를 개별 docker run으로 수동 실행하면 네트워크, 의존성(실행 순서), 환경변수, 볼륨, 포트 매핑 같은 설정이 분산되어 운영이 비효율적일 수 있다. Docker Compose는 docker-compose.yaml로 서비스 간 관계를 선언적으로 정의해 한 번에 올리고 내릴 수 있어 운영 생산성이 좋아진다.
