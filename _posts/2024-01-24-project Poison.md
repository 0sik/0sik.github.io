...
학습된 식물들의 도감 리스트/상세 페이지

도감 리스트는 전체 로딩이 아닌 무한 스크롤 기반의 점진적 로딩
...
사용자가 이미지를 업로드하면 Django는:

업로드 요청을 검증하고 저장(S3 등)

추론 작업을 작업 큐(Celery)에 등록

사용자에게는 즉시 “진행 중” 상태와 작업 식별자(task id) 를 응답

이후 AI 워커(또는 AI 서비스)가 백그라운드에서 추론을 수행하고 결과를 저장합니다.
...
랭킹(많이 조회된 순)을 설계할 때 고려할 점

“조회 수 증가”를 매 요청마다 DB에 업데이트하면 동시성 증가 시 병목이 생길 수 있습니다.
그래서 랭킹은 보통 다음 중 하나로 설계합니다.

Redis Sorted Set 등으로 실시간 집계 후 주기적으로 DB 반영(배치)

일정 시간 단위로 집계하여 정확도와 성능을 트레이드오프

구현이 어디까지였든, “트래픽 증가 시 DB 업데이트 병목을 피하기 위한 집계 전략을 고려했다”는 식으로 정리하면 운영 관점에서 강점이 됩니다.
